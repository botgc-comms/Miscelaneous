...
The Yodeck token requires permission to view and change both **Media** and **Playlists** and **Push to Screens** in the workspace that contains the Clubhouse playlist. A send creates or updates a true local full-resolution PNG, completes the signed binary upload, and waits for Yodeck to report the media state as `finished` before touching the playlist. The selected availability dates are sent in the RFC 3339 form required by Yodeck's API, from 00:00:00 on the chosen start date through 23:59:59 on the event date. Event Playbook then verifies the playlist entry and performs Yodeck's workspace-scoped bulk push with download time slots bypassed. Yodeck's terminal `completed` status is authoritative, and the returned affected-screen count is retained for diagnostics. If the playlist response does not identify a workspace, Event Playbook stops rather than risk pushing every screen in the account. URL-imported legacy items are replaced by local-upload items, duplicate playlist references are removed, and unrelated playlist items are preserved.
