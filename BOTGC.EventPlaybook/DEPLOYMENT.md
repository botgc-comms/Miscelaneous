...
9. Open the existing Web `onrender.com` URL, sign in, then use **Plugin administration** to configure Intelligent Golf and Yodeck. Enter the Yodeck API token, clubhouse playlist ID, friendly destination name and media duration there, then enable the module. Existing deployments that still supply the legacy Yodeck environment variables are migrated automatically on first startup.
...
| `YODECK_API_TOKEN` | Legacy fallback only | Normally saved securely through Plugin administration. Existing values are imported automatically while no explicit Yodeck plugin record exists. |
| `YODECK_PLAYLIST_ID` | Legacy fallback only | Normally saved through Plugin administration. Identifies the existing Clubhouse playlist in which each event's artwork is created or updated. |
| `YODECK_API_TOKEN_LABEL` | No | Label sent in Yodeck's `Token label:value` authorization header. Defaults to `event-playbook`. |
| `YODECK_PLAYLIST_NAME` | Legacy fallback only | Friendly playlist name imported until explicit plugin settings exist. Defaults to `Clubhouse`. |
| `YODECK_MEDIA_DURATION_SECONDS` | Legacy fallback only | Poster duration imported until explicit plugin settings exist. Defaults to 15 seconds. |
...
The Yodeck token requires permission to view and change both **Media** and **Playlists**, plus **Push to Screens**. A send creates or updates a true local full-resolution PNG, completes the signed binary upload, and waits for Yodeck to report the media state as `finished` before touching the playlist. The selected availability dates are sent in the RFC 3339 form required by Yodeck's API, from 00:00:00 on the chosen start date through 23:59:59 on the event date. Event Playbook then updates the existing Clubhouse playlist and calls Yodeck's account-level **Push to Screens** operation with download time slots bypassed. This refreshes content already assigned to each permitted screen; it does not assign the Clubhouse playlist to unrelated screens. Yodeck's terminal `completed` status is authoritative, and any returned affected-screen count is retained only for diagnostics. URL-imported legacy items are replaced by local-upload items, duplicate playlist references are removed, and unrelated playlist items are preserved.
...
The in-app **Plugin administration** page manages the Intelligent Golf club-site login and member-email sender identity, the Yodeck API token and clubhouse playlist settings, and a Monday.com personal API token with optional workspace and board IDs. Each plugin has an administrator-controlled on/off switch. Turning Intelligent Golf on sends the credentials and sender settings from the Web server to the private API over Render's internal network, protected by `X-Api-Key`. The API validates the login against IG, keeps its authenticated cookie session and sender settings in memory, and returns a four-hour opaque session token to the Web server. Yodeck settings are resolved for every publish or take-down operation, so saved changes and the on/off switch take effect without restarting the service. Secrets and tokens are never returned to the browser. Turning a module off retains its settings, while turning it on is rejected until its required configuration is present. The plugin settings live in `App_Data/plugin-settings.json`; secrets are encrypted and non-secret settings remain visible to administrators. Leave a secret field blank when updating other settings to retain it. **Remove credentials** deletes all stored settings for that plugin. Monday.com task synchronisation remains the next adapter to be implemented.

The Web service evaluates Event Playbook email alerts from 08:00 Europe/London and retries failed recipient digests every 30 minutes for the rest of that local day. It sends one digest per recipient. Task work is included at due-minus-two calendar days, on the due date and on every overdue day; overdue work is also included in the event organiser's digest. Every active future event with incomplete planning questions is also included in its coordinator's daily digest with its completion percentage and a link to the first unfinished planning area. The Intelligent Golf plugin must be enabled and fully configured, including its sender identity. Loading the updated Playbook once after deployment materialises the alert schedule and completion links for existing events; subsequent event, question, task, contact and deadline changes refresh it automatically.
...
- the active club question/task configuration, including approved Playbook Assistant changes, is revisioned at `App_Data/playbook-template.json`; the bundled core remains in the application image and can be restored by an administrator;
