...
public sealed class EventStatusNotificationServiceTests : IDisposable
{
    ...
    public async Task SendAsync_SendsOneCommonMessageAndDeduplicatesRecipients()
    {
        ...
        var service = CreateService(communications);
        var request = Request(
    ...
    public async Task SendAsync_RetryDoesNotResendRecipientsAlreadyDeliveredForTheDecision()
    {
        ...
        var first = await CreateService(firstClient).SendAsync(request, new Uri("https://events.example/"), CancellationToken.None);
    ...
    private EventStatusNotificationService CreateService(RecordingCommunicationsClient communications) =>
        new(
            communications,
            new TestWebHostEnvironment(_contentRoot),
            NullLogger<EventStatusNotificationService>.Instance);
