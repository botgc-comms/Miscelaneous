...
export function TeamRoster({
  ...
  return (
    <section
      className="card organiser-team roster-card"
        ...
        <section aria-label="In the team">
          ...
          <div className="roster-families">
            {families.map((family) => (
              ...
              >
                {family.length > 1 && (
                  <span className="roster-family-label">Siblings</span>
                )}
                {family.map((e) => {
                  const p = s.players.find((p) => p.id === e.playerId);
                  if (!p) return null;
          ...
          <DialogContent>
            ...
            <div className="row wrap">
              <button
                className="btn"
                disabled={
                  busy || (team.enrollmentOpen !== false && pending.length > 0)
                }
                onClick={() =>
                  void run({
                    type: 'team-directory',
              ...
              >
                {team.enrollmentOpen === false ? 'Reopen entry' : 'Close entry'}
              </button>
        ...
        <Dialog open onOpenChange={setSharing}>
          <DialogContent className="editor-dialog">
            <DialogHeader>
              ...
              <DialogDescription>
                Parents choose their child and request a place. You approve each
                application.
              </DialogDescription>
            </DialogHeader>
