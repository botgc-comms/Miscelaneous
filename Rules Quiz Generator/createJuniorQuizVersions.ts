...
function normaliseChoices(originalChoices: Choice[], juniorChoices: Choice[]): Choice[] {
...
function normaliseJuniorVersion(metadata: Metadata, response: JuniorResponse): JuniorVersion {
...
export async function createJuniorVersionForMetadata(metadata: Metadata): Promise<JuniorVersion> {
  ...
  const parsed = JSON.parse(extractTextFromResponse(response)) as JuniorResponse;
...
async function processMetadataFile(metadataPath: string, index: number, total: number, workerId: number): Promise<RunResult> {
  const startedAt = Date.now();
  const folder = path.dirname(metadataPath);
  const outputPath = path.join(folder, JUNIOR_FILE_NAME);
  const metadata = await readJsonFile<Metadata>(metadataPath);
...
async function createFailedResult(metadataPath: string, index: number, total: number, error: unknown): Promise<RunResult> {
...
function buildProgressLine(processed: number, total: number, succeeded: number, skipped: number, failed: number, startedAt: number): string {
