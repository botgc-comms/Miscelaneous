...
async function latestQuestionVersionPath(folderPath) {
...
async function nextQuestionVersion(folderPath) {
...
function normaliseQuestion(metadata, suggestion) {
    const suggestedChoices = new Map(suggestion.choices.map((choice) => [normaliseId(choice.id), choice.text.trim()]));
    return {
...
function normaliseJuniorQuestion(metadata, suggestion) {
...
function buildOfficialPrompt(metadata, instructions) {
    return [
...
function buildJuniorPrompt(metadata, officialSuggestion) {
    return [
...
export async function suggestQuestionEdit(request) {
    ...
    const metadata = latestQuestion
        ...
        : originalMetadata;
    const officialRaw = await askJson(buildOfficialPrompt(metadata, request.instructions));
    const suggestion = normaliseQuestion(metadata, officialRaw);
    const juniorRaw = await askJson(buildJuniorPrompt(metadata, suggestion));
