...
async function nextFolderPath(outputDir, title) {
...
function normaliseCorrectAnswers(value) {
    return value.map((answer) => answer.trim().toLowerCase()).filter(Boolean);
}
function normaliseMetadata(created) {
...
async function createStandardMetadata(description) {
    ...
    return normaliseMetadata(extractJson(response.output_text));
}
async function latestGeneratedImagePath(folderPath, folderName) {
...
export async function createQuizFromRuleDescription(request) {
    ...
    await fs.writeFile(path.join(folderPath, "final-prompt-v2.txt"), metadata.imagePrompt, "utf8");
    const juniorVersion = await createJuniorVersionForMetadata(metadata);
    await fs.writeFile(path.join(folderPath, "junior-version.json"), JSON.stringify({
        schemaVersion: 1,
